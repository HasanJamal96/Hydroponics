...
$d['cam_id']=$id;
    ...
    public function analytics(){
		...
		$data['sensorname']=$this->db->where('user_id',$this->session->userdata('admin_id'))->get('Sensor')->result_array();
...
//
function create_time_range24($start, $end, $interval = '30 mins', $format = '24') {
    $startTime = strtotime($start); 
    $endTime   = strtotime($end);
    $returnTimeFormat = ($format == '12')?'g:i:s A':'Y-m-d h:i:s';
...
function viewgraph($param1){
	//echo "<pre>";

	$enddate = date('Y-m-d h:'.'00:00');
	$stardate = date("Y-m-d h:"."00:00", strtotime('-24 hours', time()));
	// echo $enddate;	
	// echo "<br>";
	// echo $stardate;


	// print_r($this->create_time_range24($stardate, $enddate, '60 mins'));	   
	// echo "</pre>";
	$mysesorinfo=array();
$sensorinfo=$this->db->where('sensor_id',$param1)->get('Sensor')->row_array();
	// echo "<pre>";
	// print_r($sensorinfo);
	$x=json_decode($sensorinfo['default_value']);
	for($y=0;$y<count($x);$y++){
		$mysesorinfo[$y]=$x[$y]->label;
	}


$sensordata=$this->db->where('sensor_id',$param1)->get('sensor_data')->result_array();
// echo "<pre>";
// print_r($sensordata);
// echo "</pre>";
$time=array();
...
foreach($sensordata as $s){
	...
	// echo "</pre>";
		...
		$new=array();
		for($k=0;$k<count($x);$k++){
			$c[0]=date("Y-m-d H:i:s",strtotime($s['inserted_date']));
			$c[1]=(float)$x[$k]->val;
			array_push($new,$c);
		}
	...
	//array_push($value,(float)$x[0]->val) ;
	//array_push($time,$s['inserted_date']) ;
}
// echo "<pre>";
// print_r($newdata);
// echo ($seriesno);
...
$d['valuesdata']=$value;
$d['newdata']=$newdata;
$d['sesnor_info']=	$mysesorinfo ;  
 $d['sensorname']=$sensorinfo['name'];
// $d['adminlist']=$this->db->where('user_id',$this->session->userdata('admin_id'))->get('sensor_data')->result_array();

 $this->load->view('admin/Sensornew',$d);
}
...
function profile(){
	...
	$this->load->view('admin/editprofile',$d);
...
//  echo "</pre>";
	...
	function editoproifledata($id){
	    ...
	    if(empty($d)){
				...
				redirect(base_url().'admin/profile/'.$id);
	    }else{
