...
class Superadmin extends CI_Controller
{
	...
	public function index()
	{
        ...
        redirect(base_url().'superadmin/analytics');
...
public function analytics(){
    $d['listadmin']=($this->db->select('*')->where('type','admin')->get('users')->result_array());
    $d['listopertaor']=($this->db->select('*')->where('type','operator')->get('users')->result_array());
    $d['listsite']=($this->db->get('site')->result_array());
     $d['listsensor']=($this->db->select('*')->get('Sensor')->result_array());
...
function editadmindata($id){
    ...
    function editprofiledata(){
        ...
        $d['version']=rand(10,100);
        
        $this->db->where('id',$insert_id);
        $this->db->update('users',$d);
    $this->session->set_flashdata('success', 'Data has been updated succesfully.');
...
public function deletesite($id){
    $this->db->where('site_id',$id)->delete('adminsite');
    $this->db->where('site_id',$id)->delete('site');

 $this->session->set_flashdata('success', 'Data Has Been Removed.');
        redirect(base_url().'superadmin/site/');
